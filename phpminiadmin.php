...
function display_select($sth,$q){
    ...
    $fields_num=mysqli_num_fields($sth);
...
function db_connect($nodie=0){
    ...
    $dsn = "mysql:dbname={$DB['db']};host={$DB['host']};port={$DB['port']};charset={$DB['chset']}";
    $options = array(PDO::ATTR_PERSISTENT => 0);
    $dbh = new PDO($dsn, $DB['user'], $DB['pwd'], $options);
    $sql = "SET NAMES '{$DB['chset']}';";
    $dbh->exec($sql);
    $dbh->exec("SET sql_mode='';");
    $selectSql="SELECT * FROM our_article";
    $result=$dbh->query($selectSql);
    ...
    if ($dbh && $DB['db']) {
        $res=mysqli_select_db($DB['db'], $dbh);
        if (!$res) {
            $err_msg='Cannot select db because: '.mysqli_error();
            if (!$nodie) die($err_msg);
        }else{
...
function do_export(){
    ...
    if ($ct==1&&$_REQUEST['et']=='csv'){
        ...
        $fn=mysqli_num_fields($sth);
        for($i=0;$i<$fn;$i++){
            $m=mysqli_fetch_field($sth,$i);
